...
def helper_fetch_localmodel(model, device="cpu"):
    ...
    if max_num > 0:
        return max_num, torch.load(f"model/{mname}_{max_num}.sdict", map_location=torch.device(device))
    
def helper_save_model(model, epoch):
    try:
        name = f"model/{getmodelname(model)}_{epoch}.sdict"
...
class VisualEmbeddingsDashboard:
    def __init__(self, title="", epoch=0):
        self.fig = plt.figure()
        self.epoch = epoch
        self.embedding_ax = self.fig.add_axes([0, 0.25, 1, 0.75])
        self.plot_ax = self.fig.add_axes([0, 0, 1, 0.25])

        self.last_loss = np.array([0, 0])
        self.last_loss_v = np.array([0, 0])
        self.loss = 0

        self.title_ax = self.fig.add_axes([0.5, 0.90, 0.1, 0.1])
        self.title_ax.axis("off")
        self.title = title
        self.gettext = lambda : f"{self.title}\n{self.epoch}\n{self.loss}"
        self.text = self.title_ax.text(0, 0, self.gettext(), color="white", ha="center", va="top")

    def plot_embeddings(self,samples, mapper):
        for index, sample in enumerate(samples):
            self.embedding_ax.plot(*sample, "o", color=mapper(index))

    def plot_loss(self, epoch, loss):
        self.plot_ax.plot([self.last_loss[0], epoch], [self.last_loss[1], loss], "o-", color=(0.2, 0.5, 0.7))
        self.last_loss = [epoch, loss]
    
    def plot_loss_validation(self, epoch, loss):
        self.loss = loss
        self.plot_ax.plot([self.last_loss_v[0], epoch], [self.last_loss_v[1], loss], "o--", color=(0.7, 0.5, 0.2))
        self.last_loss_v = [epoch, loss]
    
    def update(self):
        self.text.set_text(self.gettext())
        self.fig.canvas.flush_events()
        self.fig.show()
        self.epoch += 1
    def save_image(self):
        try:
            ...
            pass
        self.fig.savefig(f"{self.title}/{self.epoch:04}.png")
